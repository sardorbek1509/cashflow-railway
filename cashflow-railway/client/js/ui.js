...
class UI {
  ...
  // ── ROOMS LIST ─────────────────────────────────────────────────
  renderRooms(rooms, currentUserId) {
    ...
    container.innerHTML = rooms.map(r => {
      const isOwner = r.hostId && currentUserId && r.hostId.toString() === currentUserId.toString();
      return `
      <div class="room-card" data-room-id="${r.roomId}">
        <div class="room-info">
          <div class="room-host">🎮 ${r.hostUsername}'s Room</div>
          <div class="room-meta room-code-hidden" title="Click to reveal">
            Code: <span class="room-code-blur" onclick="this.classList.toggle('revealed')">${r.roomId}</span>
          </div>
        </div>
        <div class="room-players">${r.players.length}/${r.maxPlayers}</div>
        <div class="room-actions">
          ${isOwner
            ? `<button type="button" class="btn btn-danger btn-sm delete-room-btn" data-room-id="${r.roomId}">🗑 Delete</button>`
            : `<button type="button" class="btn btn-secondary btn-sm join-room-btn" data-room-id="${r.roomId}">Join</button>`
          }
        </div>
      </div>
    `}).join('');
  ...
  renderRoomsList(rooms, currentUserId) {
    return this.renderRooms(rooms, currentUserId);
  }

  renderWaitingPlayers(players, hostId) {
    return this.updateWaitingPlayers(players, hostId);
  }

  renderPlayersList(playerStates, currentTurnUserId, myUserId) {
    return this.updateAllPlayers(playerStates, { players: playerStates, currentTurn: playerStates.findIndex(p => p.userId?.toString() === currentTurnUserId) });
  }

  updateStatPanel(playerState) {
    return this.updateMyFinances(playerState);
  }
  ...
  appendChatMessage(username, message, system = false) {
  ...
  showGameOver(data) {
    ...
    return this.showWinModal(data, true);
  }
