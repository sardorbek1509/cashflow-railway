...
const deleteRoom = async (req, res, next) => {
  try {
    ...
    if (room.hostId.toString() !== req.user._id.toString()) {
      return res.status(403).json({ success: false, message: 'Faqat host xonani o\'chira oladi.' });
    }
    if (room.status === 'in_progress') {
      return res.status(400).json({ success: false, message: 'O\'yin davomida xonani o\'chirib bo\'lmaydi.' });
    }
    await GameRoom.deleteOne({ roomId: req.params.roomId });
...
module.exports = { createRoom, listRooms, getRoom, getRoomState, deleteRoom };
