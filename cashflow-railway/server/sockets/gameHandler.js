...
const registerGameHandlers = (io, socket) => {
  ...
  socket.on('room:join', async ({ roomId }) => {
    try {
      ...
      if (room.status === 'finished') return socket.emit('error', { message: "Bu o'yin tugagan." });

      // Check if player is already in the room
      const alreadyInRoom = room.players.some(p => p.userId.toString() === userId);

      if (!alreadyInRoom) {
        // New player can only join waiting rooms
        if (room.status !== 'waiting') return socket.emit('error', { message: "O'yin allaqachon boshlangan." });
        if (room.players.length >= room.maxPlayers) return socket.emit('error', { message: "Xona to'lgan." });
        room.players.push({ userId, username, socketId: socket.id, isReady: false, isConnected: true });
        await room.save();
      } else {
        // Update their socket ID and connected status (host or reconnecting player)
  ...
  socket.on('room:ready', async ({ roomId }) => {
    try {
      ...
      if (allReady) {
        io.to(roomId).emit('room:all_ready', { roomId });
      }
    } catch (err) {
  ...
  socket.on('room:chat', async ({ roomId, message }) => {
    try {
      ...
      const room = await GameRoom.findOne({ roomId });
      if (!room) return;
      // Only joined players can chat
      const isPlayer = room.players.some(p => p.userId.toString() === userId);
      if (!isPlayer) return socket.emit('error', { message: 'Siz bu xonada emassiz.' });
      ...
      logger.error('room:chat error', { error: err.message });
    }
  });
  ...
  socket.on('room:delete', async ({ roomId }) => {
    try {
      ...
      if (room.hostId.toString() !== userId) {
        return socket.emit('error', { message: `Faqat host xonani o'chira oladi.` });
      }
      if (room.status === 'in_progress') {
        return socket.emit('error', { message: `Oyin davomida xonani o'chirib bo'lmaydi.` });
      }
      // Notify all players in room
      io.to(roomId).emit('room:deleted', { message: `Host xonani o'chirdi.` });
      await GameRoom.deleteOne({ roomId });
      logger.info('Room deleted', { roomId, hostId: userId });
    } catch (err) {
      logger.error('room:delete error', { error: err.message });
      socket.emit('error', { message: `Xona o'chirilib bo'lmadi.` });
    }
  });
